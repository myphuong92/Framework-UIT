...
namespace Funiture_Project.Controllers
{
    public class ProfileController : Controller
    {
        ...
        [Route("/cancel/{mahd}", Name = "CancelOrder")]
        public IActionResult Cancel(int mahd)
        {
            var hoadon = context.HoaDon.AsNoTracking()
                .Where(x => x.MaHd == mahd)
                .FirstOrDefault();
            if(hoadon.Ttdh.Equals("Đang chờ"))
            {
                ...
                context.SaveChanges();
                notyfService.Success("Hủy hóa đơn thành công");
            }
            else
            {
                notyfService.Error("Không thể hủy hóa đơn");
            }
            return RedirectToAction("Index", "Profile");
        }
    }
